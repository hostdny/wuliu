...
<div class="main-info" >
  <div class="left" id="left">
    ...
    <div class="left-top1">
      ...
      <p><span style="color:#cccccc">灰色</span>代表未做 &nbsp;&nbsp;<span style="color: green">绿色</span>代表已做</p>
    </div>
    <div class="left-tm" >
      <p><b>单选题</b></p>
    </div>
      ...
      <ul>
        <c:forEach items="${questionList.moreList}" var="moreList">
          ...
          <c:if test="${moreList.answered==0}">
            <li><a href="#" id="${moreList.no}" onclick="selectButton(${moreList.no})" style="background-color: #cccccc">${moreList.no}</a></li>
          </c:if>
        </c:forEach>
      </ul>
    </div>
      ...
      <ul>
        <c:forEach items="${questionList.judgeList}" var="judgeList">
          ...
          <c:if test="${judgeList.answered==0}">
            <li><a href="#" id="${judgeList.no}" onclick="selectButton(${judgeList.no})" style="background-color: #cccccc">${judgeList.no}</a></li>
          </c:if>
        </c:forEach>
      </ul>
    </div>
...
<script>
  ...
  function noPlus(){
    ...
    if(noId + 1 >totalNum){
      ...
      doSubmit(noOid,joinExamId,noId + 1,0);
    }
  ...
  function selectButton(no){
    var joinExamId = $("#joinExamId").val();
    var noOid = $("#noId").val();
    doSubmit(noOid,joinExamId,no,0);
  }
