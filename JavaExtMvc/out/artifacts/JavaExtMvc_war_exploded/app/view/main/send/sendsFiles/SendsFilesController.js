 ...
 */
Ext.define('ExtFrame.view.main.send.sendsFiles.SendsFilesController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.sendsFilesController',
    ...
    onClickButtonEdit: function () {
        ...
        var createOrgname = window.down('#creatOrgname');
        //仅能选择一项数据
        if (Tools.Method.IsEditData(selectRecords)) {
            window.expand();
            window.down('form').getForm().loadRecord(selectRecords[0]);
            createOrgname.setValue(selectRecords[0].data.createOrg);
        }
    },
    onClickButtonSave: function () {
        ...
        if (form.isValid()) {
            ...
            record.createTime = form.down("#createTime").getValue();
