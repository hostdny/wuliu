 ...
 */
Ext.define('ExtFrame.view.main.irs.irsEquipment.IrsEquipmentGrid', {
    extend: 'Ext.grid.GridPanel',
    alias: 'widget.irsEquipmentGrid',
    fit: true,
    stripeRows: true,
    initComponent: function () {
        ...
        var OrderField = 'name';//默认排序字段
        var OrderType = 'ASC';//默认排序类型 ASC/DESC
        me.columns = [
            {text: 'id', dataIndex: 'id', hidden: true},
            {
                text: '设备状态', dataIndex: 'status', renderer: function (v) {
                if (v == "0") {
            ...
            {text: '设备类型', dataIndex: 'type'},
            {text: '屏幕尺寸(英寸)', dataIndex: 'screenSize'},
            {
                text: '运行状态', dataIndex: 'runStatus', renderer: function (v) {
                    if (v == "0") {
        ...
        me.store = Ext.create('ExtFrame.store.User', {
            ...
            proxy: {
                type: 'ajax',
                url: Tools.Method.getAPiRootPath() + "/irsEquipment/pagedQueryByBean.do",
                reader: {
                    type: 'json',
