/*PlUpload封装 Create by sc 2015.10.28 
 在使用时需要传递自定义参数
 */
var uploader;
...
Ext.define('ExtFrame.view.extEncap.FileUpload', {
    ...
    title: '文件上传',
    uploadURL: '',// 服务器端的上传地址
    frontFlag: '',// 区分标记,用于前台区分不同模块的上传
    //自定义上传参数*/
    PlUploadOpts: {
        uniqueNames: true, //当值为true时会为每个上传的文件生成一个唯一的文件名
        fileCount: 5,//允许上传文件个数
    ...
    items: [{
        xtype: 'panel',
        html: '<div id="PlUpload_div"><p id="PlUpload_buttons">' +
            '<button id="browse" style="cursor: pointer">选择文件</button>&nbsp;&nbsp;&nbsp;' +
            '<button id="start_upload" style="cursor: pointer">开始上传</button>&nbsp;&nbsp;&nbsp;' +
            '<button id="close_upload" style="cursor: pointer">关闭</button></p><ul id="file-list"></ul></div>'
    }],
    listeners: {
        afterrender: function (me, eOpts) {
            ...
            uploader = new plupload.Uploader({
                ...
                url: me.uploadURL, //服务器端的上传页面地址
                flash_swf_url: Tools.Method.getRootPath() + '/Scripts/PlUpload/Moxie.swf', //swf文件，当需要使用swf方式进行上传时需要配置该参数
                silverlight_xap_url: Tools.Method.getRootPath() + '/Scripts/PlUpload/Moxie.xap', //silverlight文件，当需要使用silverlight方式进行上传时需要配置该参数
                chunk_size: 0, //分片上传文件时，每片文件被切割成的大小，如"200kb"，当该值为0时表示不使用分片上传功能
            ...
            uploader.bind('FilesAdded', function (uploader, files) {
                ...
                for (var i = 0, len = files.length; i < len; i++) {
                    ...
                    //构造html来更新UI
                //<a href="javascript:void(0)" class="remove" onClick="RemoveFile(\'' + file_id + '\')">移除</a>
                    var html = '<li id="file-' + files[i].id + '">' +
                        '<span id="file-span-' + files[i].id + '">0% <img class="remove" style="cursor: pointer;height: auto" src="../../../ico/delete.gif" onClick="RemoveFile(\'' + file_id + '\')" /></span>' +
            ...
            uploader.bind('UploadProgress', function (uploader, file) {
                //每个事件监听函数都会传入一些很有用的参数，
                //我们可以利用这些参数提供的信息来做比如更新UI，提示上传进度等操作
                $('#file-' + file.id + ' .progress').css('width', file.percent + '%');//控制进度条 宽度
                $('#file-' + file.id + ' .progress').css('height', '10px');//控制进度条 高度
            ...
            uploader.bind('FileUploaded', function (uploader, file, responseObject) {
                ...
                var html = '<img style="cursor: pointer;height: auto" src="../../../ico/delete.gif" onClick="DeleteFile(\'' + file.id + '\')" />';//添加删除按钮
                $(html).appendTo('#file-span-' + file.id);
            ...
            //给"关闭"按钮注册事件
            document.getElementById('close_upload').onclick = function () {
                me.close();
            }
        },
