...
var dateSelection = {
    ...
    go : function() {
        if (this.currYear == this.tmpYear && this.currMonth == this.tmpMonth) {
            ...
            this.commit();
            var month = $(".month.curr").html();
            console.info(month);
            $.ajax({
                type : 'GET',
                url : "../../../../../signAttendanceList/queryByWorkDateState.do",
                data : {
                    'month' : month
                },
                dataType : "json",
                ...
                success : function(data) {
                    $.each(data, function(i, n){
                        // 选择所有日期
                        var allDays = $('td[class=""]');
                        $.each(allDays, function(x, y){
                            console.info(y);
                            var day = $(y).children().filter("font:first").html();
                            if(n == day){
                                mOck(y, null);
                            }
                        });
